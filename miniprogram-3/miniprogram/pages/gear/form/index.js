...
Page({
 ...
 // 在 pages/gear/form/index.js 的 handleSave 函数中替换以下逻辑
 ...
 handleSave() {
  ...
  // --- 健壮性检查 (Soft Check) ---
  // 检查核心字段是否为空
  const isBrandEmpty = brandIndex === null;
  const isSeriesEmpty = !formData.series || formData.series.trim() === '';
  
  const doSave = () => {
      // --- 兜底处理 (Fallback) ---
      const finalBrand = isBrandEmpty ? '未知品牌' : brands[brandIndex];
      const finalSeries = isSeriesEmpty ? '未命名系列' : formData.series;
      ...
      const gearData = {
        ...
        price: parseFloat(formData.price) || 0, // 防止 NaN
        specs: specs, // 即使是空对象也没关系
        barcode: finalBarcode || '',
        name: `${finalBrand} ${finalSeries} ${formData.model_sku || ''}`.trim(),
        param: paramDesc.trim()
      };

      wx.showLoading({ title: '处理中...' });

      // ... (后续保存至 DB 的代码保持不变，因为数据已经安全了) ...
      // 复制之前的 DB 操作代码放这里
      if (editId) {
           ...
           wx.cloud.callFunction({
              name: 'gear_manager',
              data: { type: 'add_gear', payload: { gearData, shareToPublic } },
              success: (res) => {
                  wx.hideLoading();
              ...
              fail: () => { wx.hideLoading(); wx.showToast({ title: '错误', icon: 'none' }); }
           });
      }
  };

  // 如果有空值，弹窗警告；否则直接保存
  if (isBrandEmpty || isSeriesEmpty) {
      wx.showModal({
          title: '信息不完整',
          content: '关键信息（品牌/系列）未填写，这可能导致搜索困难。确定要强制入库吗？',
          confirmText: '强制入库',
          confirmColor: '#FF0000',
          success: (res) => {
              if (res.confirm) doSave(); // 用户执意保存
          }
      });
  } else {
      doSave(); // 信息完整，直接保存
